...
namespace UnityStandardAssets.Characters.ThirdPerson
{
	...
	public class ThirdPersonCharacter : MonoBehaviour
	{
		...
		void Start()
		{
			...
			jumpSoundSource = gameObject.AddComponent( typeof(AudioSource) ) as AudioSource;
			jumpSoundSource.playOnAwake = false;
			jumpSoundSource.clip = jumpSoundClip;
			jumpSoundSource.volume = 0.5f;
		}
		...
		public void Move(Vector3 move, bool crouch, bool jump, bool run)
		{
			...
			if (numJumps < 3 && jump && !jumpSoundSource.isPlaying) {

				if (!jumpSoundSource.isPlaying && numJumps < 2) {
			...
			if (!walkSoundSource.isPlaying && m_SecondaryIsGrounded && move.magnitude > 0) {

				walkSoundSource.Play ();
			}
			if (!m_SecondaryIsGrounded || move.magnitude <= 0) {
		...
		void UpdateAnimator(Vector3 move)
		{
			...
			//m_Animator.speed = m_AnimSpeedMultiplier * runMultiplier;
		...
		void HandleGroundedMovement(bool crouch, bool jump)
		{
			...
			if (jump && !crouch && m_Animator.GetCurrentAnimatorStateInfo(0).IsName("Grounded") && numJumps <= 2)
			{
				// jump!
